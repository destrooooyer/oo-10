...
public class map
{
	 ...
	 * requires: x, y的取值范围为[0,79]
	 * modifies: 无
	 * effects: 返回创建时(x,y)是否与上方的点相连
	 *
	 * @param x
	 * @param y
	 * @return
	 */
	public boolean is_up_connected_2(int x, int y)
	{
		return this.map_backup[x][y].up_connected;
	}
	 ...
	 * @return
	 */
	public boolean is_right_connected_2(int x, int y)
	{
		return this.map_backup[x][y].right_connected;
	}

	/**
	 * requires: x, y的取值范围为[0,79]
	 * modifies: 无
	 * effects: 返回创建时(x,y)是否与下方的点相连
	 *
	 * @param x
	 * @param y
	 * @return
	 */
	public boolean is_down_connected_2(int x, int y)
	{
		return this.map_backup[x][y].down_connected;
	}

	/**
	 * requires: x, y的取值范围为[0,79]
	 * modifies: 无
	 * effects: 返回创建时(x,y)是否与左边的点相连
	 *
	 * @param x
	 * @param y
	 * @return
	 */
	public boolean is_left_connected_2(int x, int y)
	{
		return this.map_backup[x][y].left_connected;
	}

	/**
	 * requires: x, y的取值范围为[0,79]
	 * modifies: 无
	 * effects: 返回创建时(x,y)是否与右边的点相连
	 *
	 * @param x
	 * @param y
	 * @return
	...
	public boolean add_traffic(int x, int y, String direct)
	{
				...
				y >= 0 && y < 80)
		{
			...
			temp[0] = this.is_up_connected(x, y);
			temp[1] = this.is_down_connected(x, y);
			temp[2] = this.is_left_connected(x, y);
			temp[3] = this.is_right_connected(x, y);
