...
//				}
				if (_x != dest_x || _y != dest_y)
				{
					...
					//四个点分别找最短路，更新min
					if ((flag == 0 && this._map.is_up_connected(_x, _y)) ||     //与上相连
					...
					if ((flag == 0 && this._map.is_down_connected(_x, _y)) ||     //与下相连
					...
					if ((flag == 0 && this._map.is_left_connected(_x, _y)) ||     //与左相连
					...
					if ((flag == 0 && this._map.is_right_connected(_x, _y)) ||     //与右相连
...
//						find_shortest_path(_x, _y, dest_x, dest_y);
	...
	/**
	 * requires: x1, y1, x2, y2的取值范围都是[0,79], flag为0或1
	 * modifies: 无
	 ...
	 * @param y2
	 */
	private void find_shortest_path(int x1, int y1, int x2, int y2, int flag)
	{
		//起点与终点相同
		if (x1 == x2 && y1 == y2)
		{
		...
		while (!x_list.isEmpty())
		{
			...
			if (((flag == 0 && this._map.is_left_connected(x, y)) ||     //与左相连
					(flag == 1 && this._map.is_left_connected_2(x, y)))
					&& !bo[x][y - 1])
			{
			...
			if (((flag == 0 && this._map.is_right_connected(x, y)) ||     //与右相连
					(flag == 1 && this._map.is_right_connected_2(x, y)))
					&& !bo[x][y + 1])
			{
				x_list.addLast(x);
				y_list.addLast(y + 1);
			...
			if (((flag == 0 && this._map.is_down_connected(x, y)) ||     //下相连
					(flag == 1 && this._map.is_down_connected_2(x, y)))
					&& !bo[x + 1][y])
			{
				x_list.addLast(x + 1);
				y_list.addLast(y);
...
//						System.out.println(temp_x+"\t"+temp_y);
			...
			if (((flag == 0 && this._map.is_up_connected(x, y)) ||     //与上相连
					(flag == 1 && this._map.is_up_connected_2(x, y)))
					&& !bo[x - 1][y])
			{
				x_list.addLast(x - 1);
				y_list.addLast(y);
